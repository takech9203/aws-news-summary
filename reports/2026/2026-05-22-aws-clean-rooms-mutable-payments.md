...
AWS Clean Rooms がコラボレーションに対する可変支払い設定 (Mutable Payment Configurations) をサポートしました。この機能により、コラボレーション作成後に支払い責任者を変更できるようになり、パートナー間でのコスト分担方法に柔軟性が向上します。

具体的には、SQL クエリ、PySpark ジョブ、ML モデルのトレーニングと推論ジョブ、合成データ生成など、特定のコストタイプごとに支払いを許可するパートナーを後から追加・削除できます。さらに、SQL および PySpark 分析では複数の支払い候補者を設定でき、分析送信時に実際の支払い者を選択できます。
...
- コラボレーション作成後に支払い責任者の追加・削除が可能になった
- 変更リクエストと承認のワークフローにより、安全に支払い設定を変更できるようになった
- 分析送信時に複数の支払い候補者から実際の支払い者を選択できるようになった
...
sequenceDiagram
    ...
    CR-->>MemberA: クエリ実行<br/>指定された支払い者に課金
```
...
4. **メンバーシップレベルでの支払い設定更新**
   - `UpdateMembership` API に `membershipPaymentConfiguration` パラメータを追加
   - メンバー自身が支払い責任を更新可能
...
| 日付 | サービス | 変更内容 |
|------|----------|----------|
| 2026/05/21 | [cleanrooms](https://awsapichanges.com/archive/changes/8bd61f-cleanrooms.html) | 13 updated api methods - 支払い設定変更リクエスト関連 API の追加とレスポンスへの支払い者 ID フィールド追加 |
...
3. 変更リクエストを作成する権限を持つ IAM ロールがあること
...
コラボレーション作成者が支払い責任者の変更リクエストを作成します。この例では、指定したアカウントに SQL クエリと PySpark ジョブの支払い責任を追加しています。
...
- **パートナーシップの拡大**: 支払い条件の柔軟性により、新しいパートナーとのコラボレーション開始が容易に

### 技術面

- **API ベースの制御**: 変更リクエストの作成・承認・コミットをすべて API で実行可能で、自動化に対応
...
- 支払い候補者として指定されたアカウントには適切な予算設定が必要
...
**実装例**:
```python
# 夏季キャンペーンを主導するパートナー B に支払い責任を移行
client.create_collaboration_change_request(
...
### ユースケース 3: ML モデルトレーニングのコスト分離

**シナリオ**: 広告テクノロジー企業とパブリッシャーが共同でオーディエンスセグメンテーションモデルをトレーニング。モデルトレーニングは広告テクノロジー企業が、推論は利用するパブリッシャーが支払うモデルに変更したい。
...
AWS Clean Rooms の支払い設定変更自体に追加料金は発生しません。料金はコラボレーション内で実行される分析 (SQL クエリ、PySpark ジョブ、ML ジョブ) に対して、指定された支払い者のアカウントに課金されます。

### 料金例

| コストタイプ | 料金 |
|--------|------|
| SQL クエリコンピュート | スキャンされたデータ 1 TB あたりの料金 |
| PySpark ジョブ | コンピュートユニットの使用時間に基づく料金 |
| ML モデルトレーニング | トレーニングの使用リソースに基づく料金 |
...
- **AWS Clean Rooms ML**: ML モデルのトレーニングと推論における支払い設定の変更にも対応
...
AWS Clean Rooms の可変支払い設定は、コラボレーションパートナー間のコスト分担に大きな柔軟性をもたらします。コラボレーションの再作成なしに支払い責任を変更できるため、ビジネス関係の変化や新しいユースケースの追加に迅速に対応できます。特に、複数のパートナーが参加する長期的なデータコラボレーションにおいて、コストタイプ別の細粒度な制御と承認ワークフローによる安全な変更プロセスが有用です。
