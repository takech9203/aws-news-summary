# Amazon EC2 P6-B300 インスタンス - Asia Pacific (Hyderabad) および South America (Sao Paulo) リージョンでの提供開始

**リリース日**: 2026 年 8 月 28 日
**サービス**: Amazon EC2
**機能**: P6-B300 インスタンスの Asia Pacific (Hyderabad) および South America (Sao Paulo) リージョン拡大

📊 [このアップデートのインフォグラフィックを見る](https://takech9203.github.io/aws-news-summary/20260828-amazon-ec2-p6-b300-instances-available-additional-regions.html)

## 概要

Amazon EC2 P6-B300 インスタンスが Asia Pacific (Hyderabad) および South America (Sao Paulo) リージョンで利用可能になりました。P6-B300 インスタンスは、8 基の NVIDIA Blackwell Ultra GPU を搭載し、2.1 TB の高帯域幅 GPU メモリ、6.4 Tbps の EFA ネットワーキング、300 Gbps の専用 ENA スループット、4 TB のシステムメモリを提供する最上位クラスの GPU インスタンスです。
...
今回の拡大により、P6-B300 インスタンスは US West (Oregon)、AWS GovCloud (US-East)、US East (N. Virginia)、Asia Pacific (Hyderabad、Seoul)、South America (Sao Paulo) の 6 リージョンで利用可能となりました。インドと南米では初の提供リージョンとなり、両地域の組織がデータを国内 / 域内に保持したまま、最新世代の Blackwell Ultra GPU による最高性能の AI コンピューティングを活用できるようになります。

**アップデート前の課題**

- P6-B300 インスタンスは米国 3 リージョンと Asia Pacific (Seoul) の計 4 リージョンでのみ提供されており、インドや南米地域では利用できなかった
- インドや南米に拠点を持つ組織が P6-B300 を利用するには、米国や韓国のリージョンへのデータ転送や高いネットワークレイテンシーを許容する必要があった
- データレジデンシー要件により国内でのデータ処理が求められるインドやブラジルの組織は、最新世代の Blackwell Ultra GPU を活用できなかった

**アップデート後の改善**

- Asia Pacific (Hyderabad) リージョンで P6-B300 が利用可能になり、インド国内で数兆パラメータ規模の基盤モデルのトレーニングとデプロイが可能になった
- South America (Sao Paulo) リージョンで P6-B300 が利用可能になり、南米地域で初めて最新の Blackwell Ultra GPU にアクセスできるようになった
- 計 6 リージョン体制となり、グローバルでのワークロード分散配置、データレジデンシー対応、災害復旧の選択肢が広がった
...
flowchart TD
    ...
    subgraph Regions["利用可能リージョン"]
        ...
        R4["Asia Pacific<br/>Seoul"]
        R5["Asia Pacific<br/>Hyderabad NEW"]
        R6["South America<br/>Sao Paulo NEW"]
        R1 ~~~ R2 ~~~ R3 ~~~ R4 ~~~ R5 ~~~ R6
    end
    ...
    class R5,R6 newregion
    class Workloads,W1,W2,W3 workload
```

P6-B300 インスタンスのアーキテクチャ構成と利用可能リージョンを示しています。今回の Asia Pacific (Hyderabad) と South America (Sao Paulo) の追加により、合計 6 リージョンで利用可能になりました。
...
4. **インドおよび南米地域初の提供リージョン**
   - Asia Pacific (Hyderabad) はインドで初めて P6-B300 が提供されるリージョン
   - South America (Sao Paulo) は南米で初めて P6-B300 が提供されるリージョン
...
| リージョン | リージョンコード | 位置づけ |
|-----------|----------------|---------|
| Asia Pacific (Hyderabad) | ap-south-2 | インド初の P6-B300 提供リージョン |
| South America (Sao Paulo) | sa-east-1 | 南米初の P6-B300 提供リージョン |

## 設定方法

### 前提条件

1. Asia Pacific (Hyderabad) または South America (Sao Paulo) リージョンへのアクセス権限を持つ AWS アカウント (Hyderabad はオプトインリージョンのため、事前の有効化が必要)
...
#### ステップ 1: オプトインリージョンの有効化 (Hyderabad の場合)

```bash
aws account enable-region --region-name ap-south-2
```

Asia Pacific (Hyderabad) はオプトインリージョンのため、アカウントでリージョンを有効化します。有効化には数分かかる場合があります。
...
対象リージョンにおける P6 系インスタンスの vCPU クォータを確認します。不足している場合は、Service Quotas コンソールまたは CLI からクォータの引き上げをリクエストしてください。Sao Paulo の場合は `--region sa-east-1` を指定します。
...
- **インド / 南米地域からの低レイテンシーアクセス**: 現地リージョンでの提供により、インドおよび南米の組織が低レイテンシーで最高性能の GPU コンピューティングにアクセス可能
- **データレジデンシー要件への対応**: インドやブラジルの国内にデータを保持したまま最新の Blackwell Ultra GPU で AI ワークロードを実行でき、規制やコンプライアンス要件を満たしやすくなる
- **リージョン選択の柔軟性**: 6 リージョンでの利用が可能となり、コスト、レイテンシー、コンプライアンス要件に応じた最適なリージョン選択が可能
...
- アジア太平洋地域では Hyderabad と Seoul のみの提供であり、東京リージョンなど他のアジアリージョンでは未提供
- GPU インスタンスのサービスクォータの引き上げが必要になる場合があり、承認まで時間がかかる可能性がある

### 考慮すべき点

- P6-B300 は最上位クラスのインスタンスタイプであるため、利用コストが高額になる。長期利用の場合は購入オプションの検討が必要
- Asia Pacific (Hyderabad) はオプトインリージョンのため、利用前にアカウントでのリージョン有効化が必要
...
### ユースケース 1: インド国内での大規模基盤モデルのトレーニング

**シナリオ**: インドの AI 企業が、ヒンディー語をはじめとするインド諸言語に特化した数兆パラメータ規模の基盤モデルを国内リージョンでトレーニングする。学習データを国外に転送せず、データレジデンシー要件を満たす必要がある。
...
**効果**: 8 ノード x 8 GPU = 64 GPU で合計 16.8 TB の GPU メモリを活用し、数兆パラメータ規模のモデルを効率的にトレーニング可能。EFA 6.4 Tbps のノード間通信により高いスケーリング効率を実現しつつ、データをインド国内に保持できる。

### ユースケース 2: 南米地域向け LLM 推論サービング

**シナリオ**: 企業が大規模 LLM を本番環境にデプロイし、ブラジルおよび南米地域のエンドユーザーにリアルタイムの推論サービスを低レイテンシーで提供する。
...
**効果**: 2.1 TB の GPU メモリにより超大規模モデルを単一インスタンスにロードして推論を実行でき、300 Gbps の ENA スループットにより大量のリクエストを高スループットで処理可能。米国リージョンからの提供と比較して南米のエンドユーザーへの応答レイテンシーを大幅に削減できる。

### ユースケース 3: グローバル 6 リージョンにまたがる AI プラットフォーム

**シナリオ**: グローバル企業が米国、韓国、インド、ブラジルの各リージョンで P6-B300 インスタンスを使用し、地理的な冗長性を持つ AI プラットフォームを構築する。

**効果**: 北米、アジア、南米の複数リージョンにわたるワークロード分散により、リージョン障害時のフェイルオーバーやピーク時の負荷分散が可能になる。エンドユーザーの地理的位置に基づいたルーティングにより、グローバル全体でのレイテンシーを最適化できる。
...
- Asia Pacific (Hyderabad) - ap-south-2 **[NEW]**
- South America (Sao Paulo) - sa-east-1 **[NEW]**
...
- 📊 [インフォグラフィック](https://takech9203.github.io/aws-news-summary/20260828-amazon-ec2-p6-b300-instances-available-additional-regions.html)
- [公式発表 (What's New)](https://aws.amazon.com/about-aws/whats-new/2026/08/amazon-ec2-p6-b300-instances-available-additional-regions)
...
Amazon EC2 P6-B300 インスタンスが Asia Pacific (Hyderabad) と South America (Sao Paulo) リージョンで利用可能になり、インドと南米で初めて提供されるリージョンとなりました。8 基の NVIDIA Blackwell Ultra GPU、2.1 TB の GPU メモリ、6.4 Tbps の EFA ネットワーキングを搭載した P6-B300 は、数兆パラメータ規模の基盤モデルのトレーニングやデプロイに最適です。インドやブラジルの国内でのデータ処理が求められる組織や、両地域のエンドユーザーに低レイテンシーで AI サービスを提供したい組織は、新リージョンでの P6-B300 利用を検討することを推奨します。
