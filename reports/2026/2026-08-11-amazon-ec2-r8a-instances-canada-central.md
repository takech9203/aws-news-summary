# Amazon EC2 - R8a インスタンスが Canada (Central) リージョンで利用可能に

**リリース日**: 2026 年 8 月 11 日
**サービス**: Amazon EC2
**機能**: R8a インスタンスの Canada (Central) リージョン展開

📊 [このアップデートのインフォグラフィックを見る](https://takech9203.github.io/aws-news-summary/20260811-amazon-ec2-r8a-instances-canada-central.html)

## 概要

Amazon EC2 R8a インスタンスが AWS Canada (Central) リージョンで利用可能になった。R8a インスタンスは第 5 世代 AMD EPYC プロセッサ (コードネーム Turin) を搭載し、最大周波数 4.5 GHz を実現するメモリ最適化インスタンスである。
...
- Canada (Central) リージョンでは R8a インスタンスが利用できなかった
- カナダのお客様は R7a や他のメモリ最適化インスタンスタイプを使用する必要があった
- カナダ国内のデータレジデンシー要件を満たしながら AMD EPYC Turin プロセッサの性能をメモリ最適化ワークロードで活用できなかった

**アップデート後の改善**

- Canada (Central) で R8a インスタンスが利用可能になった
- R7a からの移行で最大 30% のパフォーマンス向上と 19% のプライスパフォーマンス改善を実現できる
- SAP 認定インスタンスがカナダリージョンで利用可能になり、カナダのお客様のメモリ集約型ワークロードに対応
...
1. Canada (Central) リージョンの AWS アカウントを保有していること
...
このコマンドは Canada (Central) リージョンで r8a.xlarge インスタンスを起動します。
...
このコマンドは Canada (Central) リージョンで利用可能な R8a インスタンスタイプとスペックを表示します。
...
- **カナダリージョン対応**: カナダ国内のデータレジデンシー要件に対応しながら最新インスタンスを利用可能
- **SAP 認定**: R7a 比で 38% 多い SAPS を提供し、ミッションクリティカルな SAP ワークロードをカナダリージョンで実行可能
...
**シナリオ**: カナダ国内の企業が、大規模なデータベースワークロードを Canada (Central) リージョンで稼働させたい
...
**シナリオ**: 分散キャッシングフリートやインメモリデータベースを Canada (Central) リージョンで運用
...
**シナリオ**: カナダの企業がデータレジデンシー要件を満たしながら SAP ワークロードを R8a インスタンス上で稼働
...
今回のアップデートにより、**Canada (Central) - ca-central-1** で R8a インスタンスが利用可能になった。
...
- 📊 [インフォグラフィック](https://takech9203.github.io/aws-news-summary/20260811-amazon-ec2-r8a-instances-canada-central.html)
- [公式発表 (What's New)](https://aws.amazon.com/about-aws/whats-new/2026/08/amazon-ec2-r8a-instances-canada-central/)
...
Amazon EC2 R8a インスタンスが Canada (Central) リージョンで利用可能になり、カナダのお客様が第 5 世代 AMD EPYC プロセッサ (Turin) の性能をメモリ最適化ワークロードで活用できるようになった。R7a 比で最大 30% のパフォーマンス向上、19% のプライスパフォーマンス改善、45% のメモリ帯域幅向上を提供する。SAP 認定を取得しており、R7a 比 38% 多い SAPS を実現する。ベアメタル 2 サイズを含む 12 サイズで、SQL/NoSQL データベース、インメモリキャッシュ、リアルタイムビッグデータ分析、EDA など多様なメモリ集約型ワークロードに対応する。Canada (Central) リージョンでメモリ最適化インスタンスを運用しているお客様は、R8a への移行を検討し、パフォーマンスとコスト効率の向上を実現することを推奨する。
